...
if (!input) throw new Error('Usage: node score-candidates.js <raw-research.json>');
const research = JSON.parse(fs.readFileSync(path.resolve(input), 'utf8'));
if (!Array.isArray(research.products)) throw new Error('Research file must contain a products array.');
...
for (const product of research.products) {
  ...
  console.log(JSON.stringify({
    ...
    score: result.score,
    grade: result.grade,
    formCap: result.formCap,
    flags: result.analyzedIngredients
      .filter((ingredient) => ingredient.flag !== 'good')
      .map((ingredient) => ({ name: ingredient.name, flag: ingredient.flag })),
  }));
}
