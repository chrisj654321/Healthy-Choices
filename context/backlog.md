...
### 0. Duplicate Campbell company records — **data-integrity fix**, found 2026-07-09
`src/data/companies.js` has TWO full company records for the same real company: `campbell` (line ~645: rev $9.4B, 4 issues incl. an active 2024-2025 EPA/Clean Water Act case with admitted liability, subsidiaries incl. Kettle Brand) and `campbell-soup` (line ~8033: rev $9.6B, 2 older issues, subsidiaries incl. Cape Cod + Snack Factory). 64 products reference `campbell`, 12 reference `campbell-soup`. Worse: the `BRAND_TO_COMPANY` mapping table has the SAME brand names (Pepperidge Farm, Goldfish, Swanson, Prego, Snyder's of Hanover) listed under BOTH ids — JS silently lets the later-defined key win, so several brands are being routed to the wrong company page right now regardless of which record is "correct." Found by the Opus reviewer on the Chips product batch (2026-07-09), who fixed the 3 new Chips entries but correctly declined to touch the root cause mid-batch.
**Fix needed:** pick one canonical id, merge the real distinct sourced content from both (don't just delete one — each has real issues/subsidiaries the other lacks), repoint all product references + brand mappings, delete the loser. This is a judgment call (which lobbying/donation figures are more current, which issues to keep) worth a proper pass, possibly via `/political-analysis` re-verification — not a quick unilateral pick from whoever finds it next.
