...
The duplicate-company bug itself is FIXED: `campbell-soup` (rev $9.6B, lobbyingSpend $1.44M, politicalDonations $1.14M/55R-45D, subsidiaries incl. Cape Cod + Snack Factory) was consolidated into `campbell` (rev $9.4B, lobbyingSpend $980K, politicalDonations $430K/51R-49D) — the canonical id, since it had 5x more product references (64 vs 12) and more current issues (2024-2025 EPA/Clean Water Act case vs. 2011-2022 BPA class actions). All product `companyId` refs (76 total) and `BRAND_TO_COMPANY` mappings repointed to `campbell`; Cape Cod + Snack Factory added to its subsidiaries list so no real content was lost; the duplicate record deleted. 794/794 tests passing, SQLite rebuilt+validated.
**Still open:** I deliberately did NOT reconcile the conflicting lobbyingSpend/politicalDonations/donationSplit figures — kept `campbell`'s existing numbers rather than guess which disclosure year each side's figures came from. A real `/political-analysis` pass should verify which (if either) is Campbell's current correct figure before the founder cites either number publicly.
...
Domain (`shelfexpose.app`) bought and **live as of 2026-07-06** — Cloudflare DNS records (CNAME flattening at apex + www, both DNS-only/gray-cloud) point to Netlify, SSL provisioned, verified serving real content over HTTPS. Netlify already auto-deploys on GitHub push and currently serves only `privacy.html`/`terms.html`. Still needed: the REAL marketing site — landing page, App Store badge/link, waitlist signup (already an open P1 item in `priorities.md` — "waitlist page" was named but never built), pulls from the same brand assets as the app (icon, green palette, VISION.md copy). Reuses existing Netlify infra — not starting from zero. Founder decided (2026-07-06): build this using the Higgsfield MCP + "Motion Website Generator" skill workflow (scroll-driven, motion-design site) instead of a plain static rebuild, for Shelf Exposé's own site only — not a client-services offering. Higgsfield MCP is connected (authenticated 2026-07-06); still need the actual "Motion Website Generator" skill file from Higgsfield's dashboard before building can start. Output goes in `web/`, deploys via the existing Netlify pipeline. Schedule deliberately, don't fold into a batch session.

### 4. Mascot design + animation — **feature/creative pipeline** · founder-flagged, added 2026-07-05
A branded character for marketing video — ties directly into the LOCKED video strategy in `memory/marketing-plan.md`: synthetic AI actors only, no founder face, recurring cast currently Parent/Spouse/Child/Aisle Shopper per `marketing/videos/ai-video-style-guide.md`. A mascot would extend that cast list, not replace the no-founder-face rule. Needs: character design (visual identity, consistent with the shelf+magnifying-glass icon/green palette), a reference sheet for consistent AI-video generation (same discipline as the existing cast), and short animation tests before committing to it as a recurring video presence. Consult `marketing/videos/ai-video-style-guide.md` before designing so it slots into the existing visual world instead of clashing with it.
...
The pivot: curated product breadth stops being a goal; the durable data assets become (1) ingredient depth (Chad deep-research running as of 2026-07-28) and (2) company transparency depth — NEW layer: animal-welfare/farm-sourcing evidence (hen housing, welfare lawsuits/enforcement, single-farm sourcing, pesticide/fertilizer practices via certifications). Products analyzed live at scan; curated set = top ~20% healthiest per category, powering redirects (mechanism already exists: `getCuratedGradeABCandidates`). **Pipeline BUILT 2026-07-28: `sourcing-transparency` skill** (evidence schema + industry modules + 4-role legal chain; eggs pilot ready to run, ≤15 brands). Still open before fleet-scale: (a) run the egg pilot and review output quality; (b) ideal-product-number question answered from scan_events category demand, not picked a priori; (c) UI surfacing of `sourcing` data (separate feature build).
...
11. **Repo file audit** (founder-flagged 2026-07-22, reaffirmed same day) — rename things for clarity, remove old/dead files, sort/organize the tree. Still deferred/low-priority — founder wants this run only when he's not otherwise coding. Two pieces already done as previews: the `.easignore` cleanup (marketing/, batches/, unused mascot masters excluded from EAS uploads) and the `marketing/` folder reorg into category folders (strategy/mascot/scripts/videos/brand-assets, see decision-log 2026-07-22). Remaining scope is everything else in the repo — `src/`, `context/`, `scripts/`, etc.
