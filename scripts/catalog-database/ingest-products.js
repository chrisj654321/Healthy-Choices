 ...
 *
 * Usage:
 *   node scripts/catalog-database/ingest-products.js --batch=25
 *   node scripts/catalog-database/ingest-products.js --batch=10 --source=openfoodfacts
 *   node scripts/catalog-database/ingest-products.js --batch=10 --source=usda
 *
 * Env:
...
// Log stays at the original top-level scripts/ location (not SCRIPT_DIR) so
// it keeps appending to the existing 46MB history instead of starting a new
// file next to the script; scripts/ingestion_log.txt is gitignored/easignored
// by that exact path and Run-HealthyChoicesIngest.ps1 tails the same file.
const LOG_FILE = path.join(PROJECT_ROOT, 'scripts', 'ingestion_log.txt');
