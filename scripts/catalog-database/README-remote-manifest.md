...
`scripts/catalog-database/manifest.json` in this repo is the source of truth. Edit that file,
then publish it:
...
1. Rebuild `products.db` (existing process — see
   `scripts/catalog-database/build-products-sqlite.js`).
2. Upload it to the `Catalog` bucket (existing process — see
   `scripts/catalog-database/upload-products-db.js`), same filename or a new one.
