...
>
> 1. `node scripts/catalog-database/ingest-products.js --batch=25 --query="protein bar"`
> 2. `node scripts/catalog-database/ingest-products.js --batch=25 --query="organic snack"`
> 3. `node scripts/catalog-database/ingest-products.js --batch=25 --query="cereal"`
> 4. `node scripts/catalog-database/ingest-products.js --batch=25 --query="energy drink"`
> 5. `node scripts/catalog-database/ingest-products.js --batch=25 --source=usda --query="granola"`
>
> Script location: C:\Users\chris\HealthyChoices\scripts\catalog-database\ingest-products.js
...
- **Option B:** Run the script directly from your Windows terminal (no rate limit for local runs):
  ...
  node scripts/catalog-database/ingest-products.js --batch=25 --query="protein bar"
  ```
...
```bat
node scripts/catalog-database/ingest-products.js --batch=25 --query="protein bar"
node scripts/catalog-database/ingest-products.js --batch=25 --query="organic snack"
node scripts/catalog-database/ingest-products.js --batch=25 --query="cereal"
node scripts/catalog-database/ingest-products.js --batch=25 --query="energy drink"
node scripts/catalog-database/ingest-products.js --batch=25 --source=usda --query="granola"
```
