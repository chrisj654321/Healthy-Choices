...
# scripts/mascot-transparent-matte.py): each clip exists as a cream-background
...
for c in wave idle-loop backflip shy-blush inspecting; do
  ...
  python scripts/mascot-transparent-matte.py "$TMP/$c/master" "$TMP/$c/nobg" "$TMP/$c/rgba"
